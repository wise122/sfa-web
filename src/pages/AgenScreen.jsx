...
const AgenScreen = () => {
  ...
  const filteredOutlets = outlets.filter(o => o.name.toLowerCase().includes(search.toLowerCase()));

  return (
    <Box p="4" bg="gray.50" minH="100vh">
      <HStack justify="space-between" mb="4">
        <Heading size="lg">List Agen</Heading>
        <Button colorScheme="blue" onClick={() => {
          setFormData({
            id: '', name: '', owner: '', address: '', latitude: '',
            longitude: '', photoPath: '', ktpPhotoPath: '', segment: 'Agen'
          });
          onOpen();
        }}>
          Tambah Outlet
      ...
      <HStack mb="4">
        <Input placeholder="Cari outlet..." value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      {loading ? (
        ...
        <VStack align="stretch" spacing="3">
          ...
          {filteredOutlets.length === 0 && (
            <Text color="gray.500">Belum ada outlet Agen</Text>
          )}
        </VStack>
      )}
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="md">
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>{formData._id ? 'Edit Outlet' : 'Tambah Outlet Agen'}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <VStack spacing="3">
